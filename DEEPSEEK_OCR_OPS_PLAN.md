...
| Attention | ViT Self‑Attention | `nn.MultiheadAttention` / 自行实现 | 输入 `[B, N, C]`，N=patch 数 | 语言侧已有 `infinicore.attention`（decoder‑style） | 目前 attention 接口偏 decoder/cached KV，用于 ViT 时需在上层封装 self‑attention（Q=K=V） | 中 | 复用语言侧 MHA/MLA 组合（Linear+matmul+softmax），上层封装 ViT Block | 待封装 |
...
| Token 组合 | image_newline / view_separator 处理 | 多步 `view/cat` | 按行在末尾插入特殊 token，再展平 | 依赖 `concat`/`view` 等基础操作 + Python 逻辑 | 目前仅在 PyTorch 端有完备逻辑；InfiniCore 侧可以先用 `from_torch`/`to_torch` 混合实现 | 中 | 完全可用 Python 逻辑 + InfiniCore Tensor 组合 | 待封装 |
| Projector | MlpProjector | Linear+激活+Linear | 把 vision feature 投到 text hidden_dim | `infinicore.nn.MlpProjector` | 结构等价于两层 MLP：`Linear(C_in->hidden)` + SiLU + `Linear(hidden->C_out)`，输入 `[B,N,C_in]` 输出 `[B,N,C_out]`，参数与 PyTorch MLP 可一一对齐 | 高 | Python 模块封装，内部复用 `F.linear`/`F.silu` 等算子；如需 GELU 可后续扩展或用 Torch fallback | 已完成 |
