...
namespace infinicore::op::mean_impl::cpu {
...
void mean_contiguous_impl(Tensor out, Tensor input, long long dim, bool keepdim, LoadFn load, StoreFn store) {
    ...
    float inv_dim = 1.0f / static_cast<float>(dim_size);
    ...
    for (Size row = 0; row < rows; ++row) {
        ...
        float mean_val = acc * inv_dim;
        Size out_offset = row;
        store(out_raw, out_offset, mean_val);
    }
}

void calculate(Tensor out, Tensor input, long long dim, bool keepdim) {
    if (out->device().getType() != Device::Type::CPU) {
        throw std::runtime_error("Mean CPU kernel only supports CPU device.");
    }
    ...
    if (dtype == DataType::F32) {
        ...
        mean_contiguous_impl(out, input, dim, keepdim, load, store);
        ...
        throw std::runtime_error("Mean CPU kernel only supports F16/BF16/F32.");
    }
}

static bool registered = []() {
    Mean::dispatcher().registerDevice(Device::Type::CPU, &calculate);
    return true;
}();
